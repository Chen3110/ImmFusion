...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None,
        ...
        attention_scores[:,:,:,:self.max_position_embeddings] += attention_mask
    ...
    def MHA_GCN(self, hidden_states, attention_mask, head_mask=None,
        ...
        if self.has_graph_conv==True:
            if self.mesh_type == 'smplx':
                joints = attention_output[:,0:22,:] 
                vertices = attention_output[:,22:677,:]
                modal_tokens = attention_output[:,677:,:]

            elif self.mesh_type == 'smpl':
                joints = attention_output[:,0:24,:]
                vertices = attention_output[:,24:455,:]
                modal_tokens = attention_output[:,455:,:]

            elif self.mesh_type == 'smplh36m':
                joints = attention_output[:,0:14,:]
                vertices = attention_output[:,14:445,:]
                modal_tokens = attention_output[:,445:,:]

            vertices = self.graph_conv(vertices)
            joints_vertices = torch.cat([joints,vertices,modal_tokens],dim=1)
        else:
...
class EncoderBlock(BertPreTrainedModel):
    ...
    def forward(self, img_feats, input_ids=None, token_type_ids=None, attention_mask=None,
        ...
        embeddings = self.img_embedding(img_feats)
        # We empirically observe that adding an additional learnable position embedding leads to more stable training
        # embeddings += position_embeddings
        embeddings[:, :self.config.max_position_embeddings] += position_embeddings
