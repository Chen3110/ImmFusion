...
def build_dataset(yaml_file, args, is_train=True, scale_factor=1):
    ...
    return MeshTSVYamlDataset(args, yaml_file, is_train, False, scale_factor)
...
def make_data_loader(args, dataset, is_distributed=True, 
        is_train=True, start_iter=0, scale_factor=1):
    if isinstance(dataset, str):
        dataset = build_dataset(dataset, args, is_train=is_train, scale_factor=scale_factor)
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        pin_memory=True, collate_fn=filter_none_collate_fn
    )
    return data_loader
